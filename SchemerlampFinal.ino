...
IPAddress local_Ip(192, 168, 10, 7);
...
void loop(void) 
{
  ...
  if (!client.connected()) {
    ...
    if (client.connected()) {
      ...
      while (!client.available()) {
        Serial.println("Wachten tot startteken: Identificeer jezelf van de server");
        delay(500);
      ...
      Serial.println("Geïdentificeerd!");
  ...
  while (client.connected()) 
  {
    ...
    if (client.available()) 
    {
      ...
      char *binaryString = &message[0]; //Pointer naar de helderheid van de lamp in de message.
      uint8_t helderheid = (uint8_t)strtol(binaryString, NULL, 2); //String to long conversion, 2 -> String in binary format.
